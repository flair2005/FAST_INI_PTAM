...
void KeyFrame::MakeKeyFrame_Lite(BasicImage<byte> &im)
{
    ...
    for(int i=0; i<LEVELS; i++)
    {
        ...
        {  // .. make a half-size image from the previous level..
            ...
            halfSample(aLevels[i-1].im, lev.im);//这里其实就是在构建图像金字塔
        }
        ...
        //发现lev.vCorners居然是先按照y值的大小排序的，y值大小一样就按照x的大小排序
        for(int y=0; y<lev.im.size().y; y++)
        {
            while(v < lev.vCorners.size() && y > lev.vCorners[v].y)
                v++;
            //可以用来存储y行角点在角点列表里的偏移量
            lev.vCornerRowLUT.push_back(v);//在y行之前的角点个数v。也就是说在图片的y行及以上，已经有多少个点
        }
//        cout << "begin" << endl;
//        for (int i = 0; i < lev.vCorners.size(); i++) {
//            cout << "y: " << lev.vCorners[i].y << "  " << lev.vCorners[i].x << "  ";
//        }
//        cout << endl;
    };
}
