...
{
    //cout << "mm3BestHomography0: " << mm3BestHomography << endl;
...
//  cout << "mm3BestHomography1: " << mm3BestHomography << endl;
...
//      cout << "mm3BestHomography2: " << mm3BestHomography << endl;
...
//      cout << "mm3BestHomography3: " << mm3BestHomography << endl;
...
//  cout << "mm3BestHomography4: " << mm3BestHomography << endl;
}
...
bool HomographyInit::Compute2(vector<HomographyMatch> vMatches, double dMaxPixelError, SE3<> &se3SecondFromFirst,int i)
{
  mdMaxPixelErrorSquared = dMaxPixelError * dMaxPixelError;
  mvMatches = vMatches;

  // Find best homography from minimal sets of image matches
//  if (isnan(mm3BestHomography[0][0]) || isnan(mm3BestHomography[0][1]) || isnan(mm3BestHomography[0][2]) ||
//          isnan(mm3BestHomography[1][0]) || isnan(mm3BestHomography[1][1]) || isnan(mm3BestHomography[1][2]) ||
//          isnan(mm3BestHomography[2][0]) || isnan(mm3BestHomography[2][1]) || isnan(mm3BestHomography[2][2])) {
//      cout << "zzzzzzzzzzzzzz" << endl;
//      return false;
//  }
  BestHomographyFromMatches_MLESAC2();//先用所有的点来算一遍se3，这是初步的

  // Generate the inlier set, and refine the best estimate using this
  mvHomographyInliers.clear();
  for(unsigned int i=0; i<mvMatches.size(); i++)
    if(IsHomographyInlier(mm3BestHomography, mvMatches[i]))//删选点
      mvHomographyInliers.push_back(mvMatches[i]);
  for(int iteration = 0; iteration < 5; iteration++)
    RefineHomographyWithInliers();//重新计算se3，优化
...
void HomographyInit::BestHomographyFromMatches_MLESAC2()
{
    cout << "mm3BestHomography0: " << mm3BestHomography << endl;
    // Not many matches? Don't do ransac, throw them all in a pot and see what comes out.
    if(mvMatches.size() < 10)
