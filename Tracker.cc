...
void Tracker::TrackFrame(Image<byte> &imFrame, bool bDraw)
{
    ...
    mCurrentKF.mMeasurements.clear();
    mCurrentKF.MakeKeyFrame_Lite(imFrame);//图像金字塔+FAST角点检测+查找表
    ...
    if(mbDraw)
    {
        glDrawPixels(mCurrentKF.aLevels[0].im);//通过opengl将图片显示出来
        //将图像金字塔第0层的图片的角点显示出来
        //if(GV2.GetInt("Tracker.DrawFASTCorners",1, SILENT))  这样下面的内容就会有显示
    ...
    {
        if (isFirst) {
            cout << "初始化时间: " << mInitime << endl;
            isFirst = false;
        }
        if(mnLostFrames < 3)  // .. but only if we're not lost!
        {
//            cout << "点数量: " << mlTrails.size() << endl;
//            cout << "地图点数量: " << mMap.vpPoints.size() << endl;
            if(mbUseSBIInit)
                CalcSBIRotation();
...
//            cout << "4444444444444444444444444" << endl;
        ...
        if(mbDraw)
            RenderGrid();
        cout << "getmMapPoints" << mnFrame<<": "<< mMapMaker.getmMapPoints() << endl;

    }
    else {// If there is no map, try to make one.
//        cv::Mat image(480, 752, CV_8UC1, imFrame.data());//需要根据图片大小来调节
//        cv::Mat image(480, 640, CV_8UC1, imFrame.data());
        double t = (double)cvGetTickCount();

//        TrackForInitialMap();

        TrackForInitialMapAuto();//自动初始化

//        if (isFirst2){
//            imwrite("111111.png", image);
//            isFirst2 = false;
//        }
        //cv::Mat image(360, 640, CV_8UC1, imFrame.data());
        //imwrite("111111.jpg", image);
        //MonoRandomIniAlan2(image);
//        cv::Mat image(480, 640, CV_8UC1, imFrame.data());
//        TrackForInitialMap2(image);//使用一帧
//        if (isFirst) {

//            cv::Mat image(480, 752, CV_8UC1, imFrame.data());
...
//        cout << "tttt " << t << endl;
        mInitime2 += t/(cvGetTickFrequency()*1000000.0);
        cout << "mInitime2 " << mInitime2 << endl;
    }
...
// to define the first two key-frames. Salient(突出的) points are tracked between the two keyframes
// using cheap frame-to-frame tracking (which is very brittle(脆弱) - quick camera motion will
...
void Tracker::TrackForInitialMap()
{
    ...
    {
//        if(frameNum == 1)  // First spacebar = this is the first keyframe
        if(mbUserPressedSpacebar)  // First spacebar = this is the first keyframe
        {
            ...
            t = (double)cvGetTickCount() - t;
            cout << "t " << t << endl;
            mInitime += t/(cvGetTickFrequency()*1000000.0);
            cout << "mInitime " << mInitime << endl;
        }
        else
    ...
    if(mnInitialStage == TRAIL_TRACKING_STARTED)
    {
        ...
        //cout << "mlTrails.size()  " << mlTrails.size() << endl;
//        cout << "mlTrails.size()  " << mlTrails.size() << endl;
//        cout << "nGoodTrails  " << nGoodTrails << endl;
...
//        if(frameNum == 6)//mav_circle 27以后都不错
        if(mbUserPressedSpacebar)
        {
            ...
            int num = 0;
            //将相互匹配的角点放到vMatches里面
            for(list<Trail>::iterator i = mlTrails.begin(); i!=mlTrails.end(); i++) {
                //cout << "mlTrails[" << num++ << "]   " << i->irCurrentPos.x << "  "  << i->irCurrentPos.y << endl;
                vMatches.push_back(pair<ImageRef, ImageRef>(i->irInitialPos,
                                                            i->irCurrentPos));
            }
            cout << "vMatches.size()  " << vMatches.size() << endl;


	    //根据匹配的点，用三角化方法将3D坐标计算出来
//            cout << endl << "mFirstKF.aLevels[0].vCorners.size(): " << mFirstKF.aLevels[0].vCorners.size() << endl;
//            cout << endl << "mFirstKF.aLevels[1].vCorners.size(): " << mFirstKF.aLevels[1].vCorners.size() << endl;
//            cout << endl << "mFirstKF.aLevels[2].vCorners.size(): " << mFirstKF.aLevels[2].vCorners.size() << endl;
//            cout << endl << "mFirstKF.aLevels[3].vCorners.size(): " << mFirstKF.aLevels[3].vCorners.size() << endl;
//            cout << endl << "mCurrentKF.aLevels[2].vCorners.size(): " << mCurrentKF.aLevels[2].vCorners.size() << endl;


//            cout << endl << "mse3CamFromWorld: " << mse3CamFromWorld << endl;
//            cout << "mFirstKF: " << mFirstKF.aLevels[0].vCorners[0].x << " " << mFirstKF.aLevels[0].vCorners[0].y << endl;
//            cout << "mFirstKF: " << mFirstKF.aLevels[0].vCorners[10].x << " " << mFirstKF.aLevels[0].vCorners[10].y << endl;
//            cout << "mFirstKF: " << mFirstKF.aLevels[0].vCorners[50].x << " " << mFirstKF.aLevels[0].vCorners[50].y << endl;
//            cout << "mCurrentKF: " << mCurrentKF.aLevels[0].vCorners[0].x << " " << mCurrentKF.aLevels[0].vCorners[0].y << endl;
//            cout << "mCurrentKF: " << mCurrentKF.aLevels[0].vCorners[10].x << " " << mCurrentKF.aLevels[0].vCorners[10].y << endl;
//            cout << "mCurrentKF: " << mCurrentKF.aLevels[0].vCorners[50].x << " " << mCurrentKF.aLevels[0].vCorners[50].y << endl;
...
// 取代原始两帧手动初始化方法，转而采用自动多帧初始化的方法
void Tracker::TrackForInitialMapAuto()
{
    frameNum++;
    // MiniPatch tracking threshhold.
    static gvar3<int> gvnMaxSSD("Tracker.MiniPatchMaxSSD", 100000, SILENT);
    ...
    if(mnInitialStage == TRAIL_TRACKING_NOT_STARTED)
    {
//        if(frameNum == 20)  // First spacebar = this is the first keyframe
//        {
//            double t = (double)cvGetTickCount();
...
//        else
//            mMessageForUser << "Point camera at planar scene and press spacebar to start tracking for initial map." << endl;
//            cout << "TRAIL_TRACKING_NOT_STARTED " << "frameNum: " << frameNum << endl;
        return;
    }

    if(mnInitialStage == TRAIL_TRACKING_STARTED)
    {
//        cout << "TRAIL_TRACKING_STARTED " << "frameNum: " << frameNum << endl;
        int nGoodTrails = TrailTracking_Advance();  // This call actually tracks the trails
//        cout << "mlTrails.size()  " << mlTrails.size() << endl;
        cout << "nGoodTrails  " << nGoodTrails << endl;
...
//                double m = (i->irInitialPos.x - i->irCurrentPos.x)*(i->irInitialPos.x - i->irCurrentPos.x)
//                        + (i->irInitialPos.y - i->irCurrentPos.y)*(i->irInitialPos.y - i->irCurrentPos.y);
//                avg += sqrt(m);
...
//            cout<< "bbbbbbbbbbbbb " << endl;
            ...
            for(list<Trail>::iterator i = mlTrails.begin(); i!=mlTrails.end(); i++) {
                ...
                double m = (i->irInitialPos.x - i->irCurrentPos.x)*(i->irInitialPos.x - i->irCurrentPos.x)
                        + (i->irInitialPos.y - i->irCurrentPos.y)*(i->irInitialPos.y - i->irCurrentPos.y);
                v.push_back(sqrt(m));
//                Vector<2> vecPoint(i->irInitialPos.x - i->irCurrentPos.x, i->irInitialPos.y - i->irCurrentPos.y);
...
//            cout << "vMatches.size()  " << vMatches.size() << endl;
        ...
        //根据匹配的点，用三角化方法将3D坐标计算出来
//            mMapMaker.InitFromStereo(mFirstKF, mCurrentKF, vMatches, mse3CamFromWorld);  // This will take some time!
//            if (!mMapMaker.InitFromStereo(mFirstKF, mCurrentKF, vMatches, mse3CamFromWorld)) {
//                cout << "InitFromStereo  " << endl;
//                Reset();
//                return;
//            }


//            cout << endl << "mFirstKF.se3CfromW: " << mFirstKF.se3CfromW << endl;
//            cout << endl << "mCurrentKF.se3CfromW: " << mCurrentKF.se3CfromW << endl;
//            cout << endl << "mse3CamFromWorld: " << mse3CamFromWorld << endl;
//            cout << endl << "mFirstKF.aLevels[0].vCorners.size(): " << mFirstKF.aLevels[0].vCorners.size() << endl;
//            cout << endl << "mFirstKF.aLevels[1].vCorners.size(): " << mFirstKF.aLevels[1].vCorners.size() << endl;
//            cout << endl << "mFirstKF.aLevels[2].vCorners.size(): " << mFirstKF.aLevels[2].vCorners.size() << endl;
//            cout << endl << "mFirstKF.aLevels[3].vCorners.size(): " << mFirstKF.aLevels[3].vCorners.size() << endl;
//            cout << endl << "mCurrentKF.aLevels[2].vCorners.size(): " << mCurrentKF.aLevels[2].vCorners.size() << endl;
//            if (!mMapMaker.InitFromStereoFAST2(mFirstKF, mCurrentKF, vMatches, mse3CamFromWorld)) {
////                cout << "InitFromStereo  " << endl;
//                ResetFAST();//这里是不需要重新计算第一帧的
//                mnInitialStage = TRAIL_TRACKING_STARTED;
//                return;
//            } else {
//                mMapMaker.InitFromStereoFAST(mFirstKF, mCurrentKF, vMatches, mse3CamFromWorld);
//            }

//            std::vector<CVD::ImageRef> mFirstKFLV0;
//            for (int i = 0; i < mFirstKF.aLevels[0].vCorners.size(); i++) {
//                mFirstKFLV0.push_back(mFirstKF.aLevels[0].vCorners[i]);
//            }
//            std::vector<CVD::ImageRef> mFirstKFLV1;
//            for (int i = 0; i < mFirstKF.aLevels[1].vCorners.size(); i++) {
//                mFirstKFLV1.push_back(mFirstKF.aLevels[1].vCorners[i]);
//            }
//            std::vector<CVD::ImageRef> mFirstKFLV2;
//            for (int i = 0; i < mFirstKF.aLevels[2].vCorners.size(); i++) {
//                mFirstKFLV2.push_back(mFirstKF.aLevels[2].vCorners[i]);
//            }
//            std::vector<CVD::ImageRef> mFirstKFLV3;
//            for (int i = 0; i < mFirstKF.aLevels[3].vCorners.size(); i++) {
//                mFirstKFLV3.push_back(mFirstKF.aLevels[3].vCorners[i]);
//            }

//            std::vector<CVD::ImageRef> mCurrentKFLV0;
//            for (int i = 0; i < mCurrentKF.aLevels[0].vCorners.size(); i++) {
//                mCurrentKFLV0.push_back(mCurrentKF.aLevels[0].vCorners[i]);
//            }
//            std::vector<CVD::ImageRef> mCurrentKFLV1;
//            for (int i = 0; i < mCurrentKF.aLevels[1].vCorners.size(); i++) {
//                mCurrentKFLV1.push_back(mCurrentKF.aLevels[1].vCorners[i]);
//            }
//            std::vector<CVD::ImageRef> mCurrentKFLV2;
//            for (int i = 0; i < mCurrentKF.aLevels[2].vCorners.size(); i++) {
//                mCurrentKFLV2.push_back(mCurrentKF.aLevels[2].vCorners[i]);
//            }
//            std::vector<CVD::ImageRef> mCurrentKFLV3;
//            for (int i = 0; i < mCurrentKF.aLevels[3].vCorners.size(); i++) {
//                mCurrentKFLV3.push_back(mCurrentKF.aLevels[3].vCorners[i]);
//            }
//            cout << "mFirstKF: " << mFirstKF.aLevels[0].vCorners[0].x << " " << mFirstKF.aLevels[0].vCorners[0].y << endl;
//            cout << "mFirstKF: " << mFirstKF.aLevels[0].vCorners[10].x << " " << mFirstKF.aLevels[0].vCorners[10].y << endl;
//            cout << "mFirstKF: " << mFirstKF.aLevels[0].vCorners[50].x << " " << mFirstKF.aLevels[0].vCorners[50].y << endl;
...
//            cout << "mCurrentKF: " << mCurrentKF.aLevels[0].vCorners[10].x << " " << mCurrentKF.aLevels[0].vCorners[10].y << endl;
//            cout << "mCurrentKF: " << mCurrentKF.aLevels[0].vCorners[50].x << " " << mCurrentKF.aLevels[0].vCorners[50].y << endl;
            if (!mMapMaker.InitFromStereoFAST(mFirstKF, mCurrentKF, vMatches, mse3CamFromWorld)) {
                cout << "InitFromStereo  " << endl;
                ResetFAST();//这里是不需要重新计算第一帧的

//                mFirstKF.aLevels[0].vCorners.clear();
//                for (int i = 0; i < mFirstKFLV0.size(); i++) {
//                    mFirstKF.aLevels[0].vCorners.push_back(mFirstKFLV0[i]);
//                }
//                mFirstKF.aLevels[1].vCorners.clear();
//                for (int i = 0; i < mFirstKFLV1.size(); i++) {
//                    mFirstKF.aLevels[1].vCorners.push_back(mFirstKFLV1[i]);
//                }
//                mFirstKF.aLevels[2].vCorners.clear();
//                for (int i = 0; i < mFirstKFLV2.size(); i++) {
//                    mFirstKF.aLevels[2].vCorners.push_back(mFirstKFLV2[i]);
//                }
//                mFirstKF.aLevels[3].vCorners.clear();
//                for (int i = 0; i < mFirstKFLV3.size(); i++) {
//                    mFirstKF.aLevels[3].vCorners.push_back(mFirstKFLV3[i]);
//                }

//                mCurrentKF.aLevels[0].vCorners.clear();
//                for (int i = 0; i < mCurrentKFLV0.size(); i++) {
//                    mCurrentKF.aLevels[0].vCorners.push_back(mCurrentKFLV0[i]);
//                }
//                mCurrentKF.aLevels[1].vCorners.clear();
//                for (int i = 0; i < mCurrentKFLV1.size(); i++) {
//                    mCurrentKF.aLevels[1].vCorners.push_back(mCurrentKFLV1[i]);
//                }
//                mCurrentKF.aLevels[2].vCorners.clear();
//                for (int i = 0; i < mCurrentKFLV2.size(); i++) {
//                    mCurrentKF.aLevels[2].vCorners.push_back(mCurrentKFLV2[i]);
//                }
//                mCurrentKF.aLevels[3].vCorners.clear();
//                for (int i = 0; i < mCurrentKFLV3.size(); i++) {
//                    mCurrentKF.aLevels[3].vCorners.push_back(mCurrentKFLV3[i]);
//                }
...
int Tracker::TrailTracking_Advance()
{
    ...
    for(list<Trail>::iterator i = mlTrails.begin(); i!=mlTrails.end();)
    {
        ...
        bool bFound = trail.mPatch.FindPatch(irEnd, lCurrentFrame.im, 10, lCurrentFrame.vCorners);//匹配关键点（角点）
        if(bFound)
        {
...
void Tracker::MonoRandomIniAlan2(cv::Mat src)
{
    TrailTracking_Start();
    mMapMaker.InitFromStereoForFast(mlTrails, src, mFirstKF, mCurrentKF, mse3CamFromWorld);  // This will take some time!
        ...
        if (numVps>0) {
            ...
            for(size_t i=0; i<mInitialFrame.mvKeysUn.size();i++)
            {
                ...
                for (int vi=0; vi<numVps; vi++){
                    dist += sqrt((mInitialFrame.mvKeysUn[i].pt.x-mvVanishingPoints[vi].at<float>(0,0))*(mInitialFrame.mvKeysUn[i].pt.x-mvVanishingPoints[vi].at<float>(0,0))+(mInitialFrame.mvKeysUn[i].pt.y-mvVanishingPoints[vi].at<float>(0,1))*(mInitialFrame.mvKeysUn[i].pt.y-mvVanishingPoints[vi].at<float>(0,1)));
                }
                if (dist>maxDist) {
                    maxDist = dist;
                }
...
void Tracker::TrackForInitialMap2(cv::Mat src)
{
    frameNum ++;
    // MiniPatch tracking threshhold.
    static gvar3<int> gvnMaxSSD("Tracker.MiniPatchMaxSSD", 100000, SILENT);
...
//            }
        //根据匹配的点，用三角化方法将3D坐标计算出来
//            mMapMaker.InitFromStereo2(mlTrails, src, mFirstKF, mCurrentKF, vMatches, mse3CamFromWorld);  // This will take some time!
            mMapMaker.InitFromStereo2(mlTrails, src, mCurrentKF, mse3CamFromWorld);  // This will take some time!
            mnInitialStage = TRAIL_TRACKING_COMPLETE;
