...
//added by zyx copyed from tracker.h
...
public:
  ...
  //added by zyx
  bool InitFromStereoForFast(std::list<Trail> mlTrails, cv::Mat &src, KeyFrame &kFirst, KeyFrame &kSecond,
              SE3<> &se3CameraPos);
  bool InitFromStereo2(std::list<Trail> mlTrails, cv::Mat src, //KeyFrame &kF,
                                KeyFrame &kS,
                                //std::vector<std::pair<CVD::ImageRef, CVD::ImageRef> > &vTrailMatches,
                                SE3<> &se3TrackerPose);
  bool InitFromStereoFAST(KeyFrame &kFirst, KeyFrame &kSecond,
              std::vector<std::pair<CVD::ImageRef, CVD::ImageRef> > &vMatches,
              SE3<> &se3CameraPos);

  void AddSomeMapPointsForFast(std::vector<CVD::ImageRef> &vMapPointsCandidates, int nLevel);
  std::vector<std::vector<cv::Point> > mvLineSegments;
  std::vector<std::vector<std::vector<cv::Point> > > mvLineSegmentsClusters;
  //std::vector<CVD::Image<CVD::byte> > mvVanishingPoints;
  std::vector<cv::Mat> mvVanishingPoints;
  bool hasNewDetectedVPs;
//  static bool less_int(MapPoint* a,MapPoint* b);

  int getmMapPoints() {return mMap.vpPoints.size();}
...
//  void MonoRandomIniAlan2(cv::Mat src);
  void DetectVanishingPoints(cv::Mat src, std::vector<cv::Mat> &vps);
  void LineDetect( cv::Mat image, double thLength, std::vector<std::vector<cv::Point> > &lines );
  bool BundleAdjustAllFAST();
  bool BundleAdjustFAST(std::set<KeyFrame*>, std::set<KeyFrame*>, std::set<MapPoint*>, bool);
